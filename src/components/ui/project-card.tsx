		...
		<>
			{props.isFirst ? (
				<motion.div
					...
					className='group relative glass-card bg-white/70 dark:bg-white/5 border border-white/50 dark:border-white/5 rounded-2xl overflow-hidden hover:border-blue-300/60 dark:hover:border-blue-500/30 transition-all duration-300 light-shadow hover:light-shadow-hover dark:shadow-none h-full cursor-pointer'
				>
						...
						<div className='relative bg-slate-50/50 dark:bg-neutral-900/50 border-t md:border-t-0 md:border-l border-white/60 dark:border-white/5 h-72 md:h-auto overflow-hidden'>
							...
							{props.image.length > 0 ? (
								<div className='absolute h-fit inset-x-8 top-8 bottom-0 bg-card rounded-t-xl border border-border/60 dark:border-neutral-800 shadow-xl transform translate-y-8 group-hover:translate-y-6 transition-transform duration-500'>
									...
									<div className='p-4'>
										<img
											src={cloudinaryResize(props.image[0].url, 1000)}
				...
				>
					{props.image.length > 0 && (
						<div className='absolute inset-0 z-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500'>
							<img
								src={cloudinaryResize(props.image[0].url, 800)}
								alt={props.image[0].alt}
								width={800}
								height={365}
								loading='lazy'
					...
					<div className='p-6 relative z-10 flex flex-col h-full justify-between'>
						...
						<div
							className='flex gap-2 items-center mt-auto pt-4 border-t border-border/50 dark:border-neutral-800/50 group-hover:border-white/10 transition-colors'
			...
			<ProjectDialog open={open} onOpenChange={setOpen} {...props} />
		</>
	);
}
