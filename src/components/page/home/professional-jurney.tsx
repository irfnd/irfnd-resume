	...
	return (
		<section>
			<div className='flex items-center justify-between mb-8'>
				<h2 className='text-lg font-semibold text-slate-900 dark:text-white tracking-tight'>{experience.title}</h2>
				<div className='h-px bg-slate-200 dark:bg-neutral-800 flex-1 ml-6' />
			</div>

			<div className='relative border-l border-slate-200 dark:border-neutral-800 ml-3 space-y-12 pb-4'>
				{experience.jobs.map((job, index) => (
					<div key={index} className='relative pl-6 md:pl-10 group'>
						...
						<div className='flex flex-col sm:flex-row sm:items-start sm:justify-between mb-4 gap-2'>
							<div>
								<h3 className='text-lg font-semibold text-slate-900 dark:text-white tracking-tight'>{job.mainPosition}</h3>
							...
							<div
								...
								className={cn(
									...
									descArr.length > 1 &&
										...
										'mb-0 pt-6 border-t border-dashed border-slate-200 dark:border-neutral-800/60',
								)}
							>
								{descArr.length > 1 && (
									<div className='flex items-center gap-2 mb-3'>
										{desc.icon ? <desc.icon className='size-3.5 text-blue-600 dark:text-blue-400' /> : null}
								...
								{desc.summary.map((summary, summaryIndex) => (
									<p key={summaryIndex} className='text-sm text-slate-600 dark:text-neutral-400 font-light leading-relaxed mb-4'>
